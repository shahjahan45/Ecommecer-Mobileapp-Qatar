...
void main() {
  ...
  testWidgets('checkout can select card and reach order confirmation',
      (tester) async {
    ...
    await tester.enterText(
        find.byKey(const Key('checkout-address-name')), 'Phase 13 Customer');
    await tester.enterText(
        find.byKey(const Key('checkout-address-mobile')), '55512345');
    await tester.enterText(
        find.byKey(const Key('checkout-address-line')), 'Doha, Qatar');
    ...
    await tester.pumpAndSettle();
    await tester
        .tap(find.byKey(const ValueKey<String>('checkout-payment-card')));
    await tester.pumpAndSettle();
